...
******************************************************************************/
since r18213;  // bag o' tricks tracked in mafia
import <zlib.ash>
...
void build_blacklist() {
   ...
   temp_blacklist_inc("skill 7290", 10 - to_int(get_property("_macrometeoriteUses")));   // cap macrometeorite at remaining uses
...
int timeshappened(string occurrence) { int res;
   foreach i,b in happenings[m,cid,occurrence] res += 1;
   return res;
}
...
boolean set_happened(string occurrence, int when, boolean playeraction) {
   ...
   switch (occurrence) {
      ...
      case "skill 19": case "skill 2043": case "skill 7166": case "skill 15009": case "skill 20007": case "item 2462": case "item 9481": set_happened("attracted",when,false); break;
      ...
      switch (rec.type) {
        ...
        // filter conditional drops
         case "c": if (item_type(rec.drop) == "shirt" && !have_skill($skill[torso awaregness])) break; // skip shirts if applic
            ...
            if (rec.drop == $item[beer lens] && get_property("_beerLensDrops").to_int() > 2) break;   // can only get 3 a day
            // include the rest????
        // normal, no pp
...
vprint("1 HP costs "+rnum(meatperhp,3)+entity_decode("&mu;")+". ( "+rnum(my_stat("hp"))+" / "+my_maxhp()+" )","#880000",9);
...
vprint("1 MP costs "+rnum(meatpermp,3)+entity_decode("&mu;")+". ( "+rnum(my_stat("mp"))+" / "+my_maxmp()+" )","#000088",9);
...
spread m_regular() {               // damage dealt by monster attack
   ...
   switch (m) {                    // unique damage formulas
      ...
      case $monster[ninja snowman assassin]: spread res;
         ...
         return res;
      case $monster[blazing bat]: return to_spread((max(0,monster_stat("att") - my_defstat()) + 500).to_string()+" hot");
     ...
     // Batfellow
      case $monster[vicious plant creature]: return to_spread("4*(100-batstenchresistance)/100 stench");     // low crime
      case $monster[vine-controlled botanist]: return to_spread("3*(100-batstenchresistance)/100 stench");
      case $monster[giant mosquito]: return to_spread("max(1,4-batarmor)");
      case $monster[giant leech]: return to_spread("max(1,3-batarmor)");
      case $monster[walking skeleton]:
      case $monster[lovestruck goth dude]: return to_spread("4*(100-batspookyresistance)/100 spooky");
      case $monster[Kudzu]: return to_spread("10*(100-batstenchresistance)/100 stench");
      case $monster[Mansquito]: return to_spread("10-batarmor");
      case $monster[Miss Graves]: return to_spread("10*(100-batspookyresistance)/100 spooky");
      case $monster[former inmate]: return to_spread("11*(100-batspookyresistance)/100 spooky");             // medium crime
      case $monster[plumber's helper]: return to_spread("9*(100-batstenchresistance)/100 stench");
      case $monster[liquid plumber]:
      case $monster[very [adjective] henchman]:
      case $monster[very [adjective] henchwoman]: return to_spread("9-batarmor");
      case $monster[The Plumber]: return to_spread("20*(100-batstenchresistance)/100 stench");
      case $monster[The Author]: return to_spread("20*(100-batspookyresistance)/100 spooky");
      case $monster[The Mad Libber]: return to_spread("20-batarmor");
      case $monster[inquisitee]: return to_spread("15*(100-batspookyresistance)/100 spooky");                // high crime
      case $monster[clockwork man]:
      case $monster[trivia researcher]: return to_spread("15-batarmor");
      case $monster[serial arsonist]: return to_spread("15*(100-batheatresistance)/100 hot");
      case $monster[burner]: return happened("skill 7257") ? to_spread(1) : to_spread("15*(100-batheatresistance)/100 hot");
      case $monster[time bandit]: return happened("skill 7257") ? to_spread(3) : to_spread("11-batbulletproofing");
      case $monster[Mr. Burns]: return to_spread("30*(100-batheatresistance)/100 hot");
      case $monster[Doc Clock]:
      case $monster[The Inquisitor]:
      case $monster[The Jokester]: return to_spread("30-batarmor");
      // minions depend on weapons
      case $monster[former guard]:
      case $monster[common criminal]:
      case $monster[uncommon criminal]:
      case $monster[rare criminal]:
      case $monster[low-level mook]:
      case $monster[mid-level mook]:
      case $monster[high-level mook]: if (contains_text(page,"mookbody_leadpipe.gif")) return to_spread("5-batarmor");
         else if (contains_text(page,"mookbody_knife.gif")) return to_spread("7-batarmor");
         else if (contains_text(page,"mookbody_shotgun.gif")) return to_spread("5-batbulletproofing");
         else if (contains_text(page,"mookbody_pistol.gif")) return to_spread("9-batbulletproofing");
         else if (contains_text(page,"mookbody_machinegun.gif")) return to_spread("max(1,3-batbulletproofing)*5");
         else return to_spread(contains_text(m.to_string(),"criminal") ? 3 : 1);
   }
   spread res;
...
advevent m_event(float att_mod, float stun_mod) {      // monster event -- attack + retal, factored by hitchance
   ...
   switch (m) {                   // monster special moves, not factored by hitchance
      ...
      case $monster[Mansquito]: res.dmg = to_spread("-1"); break;
      ...
      case $monster[mining grobold]: if (round % 2 == 1) res.pdmg[$element[none]] += 0.9*my_maxhp(); break;
   }
   ...
   if (aid.find()) switch (aid.group(1)) {
      ...
      case "skill": if (mdata.noskills > 0) through *= max(0, 1.0 - mdata.noskills);
         switch (to_int(aid.group(2))) {
            case 2003: through *= 1.0 - mdata.dodge; case 1: case 7097: break; // beak, head, turtle*7 tails all have regular hitchance
            ...
            case 1004: case 1005: case 1038: through *= 1.0 - mdata.dodge;  // ls, lts, northern explosion
            case 7096: if (my_class() == $class[seal clubber] && item_type(equipped_item($slot[weapon])) == "club") return through;   // lts, bashing slam smash
            ...
            case 2015: attack += min(my_level()*2,20.0);                    // kneebutt
            case 2005: attack = 0; through *= 1.0 - mdata.dodge; break;     // shieldbutt
...
void addopt(advevent a, float meatcost, int mpcost) {
   ...
   switch (m) {
      ...
      case $monster[LOV Engineer]: if (dmg_dealt(a.dmg) > 0 && !a.id.contains_text("skill")) a.meat -= item_val($item[LOV Elixir #6]); break;  // hopefully this will convince it?
...
void set_fvars(boolean eachround) {   // function to bring most of these to one place
   if (limit_mode() == "batman") {    // load all Batfellow stats into fvars
      matcher batbits = create_matcher(";?(.+?)=(\\d+)", replace_string(to_lower_case(get_property("batmanStats")),"-",""));
      while (batbits.find()) fvars[to_string(replace_string(batbits.group(1)," ",""))] = to_float(batbits.group(2));
   }
...
void build_items() {
   ...
   foreach it,fields in factors["item"] {
      ...
      rate = item_val(to_item(it))*to_int(!to_item(it).combat_reusable);    // cost: item_value, unless reusable
      if (fields.special.contains_text("custom attract") && (have_effect($effect[on the trail]) > 0 || happened("attracted"))) continue;
      switch (it) {
         ...
         case 6714: if (!contains_text(m, "pygmy")) continue;
            int hun = (monster_hp()/100)%10;
            int ten = (monster_hp()/10)%10;
            if (hun < 6 && ten < 6 && monster_hp()%10 < 6) fields.dmg = hun*64 + ten*4;
            break;                                                                            // short calculator         
...
void build_skillz() {
   ...
   void add_skill(int sk, combat_rec fields) {
      ...
      switch (m) {
         ...
         case $monster[flock of every birds]:
         case $monster[zombie homeowners' association (hard mode)]:
         ...
         case $monster[LOV Engineer]: if (!is_spell(to_skill(sk))) return; break;
      }
      ...
      switch (sk) {
        ...
        case 7255: d = to_spread("(batpunch+batpunchmodifier+"+to_string(timeshappened("skill 7256"))+")*batpunchmultiplier"); break;     // Bat-Punch/Kick get stronger as you alternate; for now, add the first bonus
        case 7256: d = to_spread("(batkick+batkickmodifier+"+to_string(timeshappened("skill 7255"))+")*batkickmultiplier"); break;
...
boolean enqueue(advevent a, boolean allowfunk) {     // handle inserts/auto-funk
   ...
   if (have_equipped($item[mime army infiltration glove]) && my_primestat() != $stat[moxie] && a.id == "pickpocket") { act(steal()); return true; }  // KoL bug: "pickpocket" unrecognized with glove on
...
string batround() {
   ...
   switch (my_fam()) {
      case $familiar[he-boulder]: if (have_effect($effect[everything looks yellow]) > 0 || !list_contains(getvar("BatMan_yellow"),m)) break;
